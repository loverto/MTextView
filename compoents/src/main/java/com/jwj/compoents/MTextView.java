...
public class MTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private int measureContentHeight(int width) {
        ...
        int maxLines = getMaxLines();
        TextUtils.TruncateAt elipsize =  getEllipsize();
        boolean isTruncat = false;
        for (int i = 0; i < obList.size(); i++) {
            ...
            if (width - drawedWidth < obWidth || splitFlag) {
                if (contentList.size()+1>=maxLines){
                   isTruncat = true;
                    break;
                }
        ...
        // 处理需要截取的一行
        if (isTruncat && lineWidthMax >= drawedWidth&& elipsize.equals(TextUtils.TruncateAt.END)&& line != null && line.line.size() > 0){
            // TODO 暂不支持 图文混排 末尾
            Object lineStringObject = line.line.get(line.line.size()-1);
            String text = lineStringObject.toString().subSequence(0, lineStringObject.toString().length() - 1) + "...";
            line.line.remove(lineStringObject);
            line.line.add(text);
        }
